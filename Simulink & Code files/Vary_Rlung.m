...
%%
R_lung = [1/100 1/200 1/300];
C_lung = 20;
R_leak = 60/1000;
R_hose = 4.5/1000;
wn = 2*pi*30;

for i = 1:length(R_lung)
    A_h(i) = -(1/R_hose + 1/R_leak)/(R_lung(i)*C_lung*(1/R_lung(i)+1/R_hose+1/R_leak));
    B_h(i) = (1/R_hose)/(R_lung(i)*C_lung*(1/R_lung(i)+1/R_hose+1/R_leak));
    C_h(:,i) = [( 1/R_lung(i)/(1/R_lung(i)+1/R_hose+1/R_leak)),  -(1/R_hose+1/R_leak)/(R_lung(i)*(1/R_lung(i)+1/R_hose+1/R_leak))]';
    D_h(:,i) = [ (1/R_hose/(1/R_lung(i)+1/R_hose+1/R_leak)) , (1/R_hose / (R_lung(i)*(1/R_lung(i)+1/R_hose+1/R_leak)))]';
end
%%
s = tf("s");
%Blower B_s
...
for i = 1:3
    ...
    fprintf("Patient hose transfer function for R_lung %f is: \n",R_lung(i))
    H_s(:,i)
    
    fprintf("Overall plant transfer function for R_lung %f is  is : \n",R_lung(i))
    B_s*H_s(:,i)
end
...
xlabel("Time[s]")
ylabel("Paw[mbar]")
title("Effect of Varying RLung on Paw")
for i = 1:length(R_lung)
...
for i = 1:length(R_lung)
    ...
    xlabel("Time[s]")
    ylabel("Qpat[mL/min]")
    title("Effect of Varying RLung on Qpat")
    hold on
end
grid on
hold off
