...
R_hose = 4.5/1000;
wn = 2*pi*30;
for i = 1:length(C_lung)
    A_h(i) = -(1/R_hose + 1/R_leak)/(R_lung*C_lung(i)*(1/R_lung+1/R_hose+1/R_leak));
    B_h(i) = (1/R_hose)/(R_lung*C_lung(i)*(1/R_lung+1/R_hose+1/R_leak));
    C_h(:,i) = [( 1/R_lung/(1/R_lung+1/R_hose+1/R_leak)),  -(1/R_hose+1/R_leak)/(R_lung*(1/R_lung+1/R_hose+1/R_leak))]';
    D_h(:,i) = [ (1/R_hose/(1/R_lung+1/R_hose+1/R_leak)) , (1/R_hose / (R_lung*(1/R_lung+1/R_hose+1/R_leak)))]';
end
%%
s = tf("s");
%Blower B_s
...
for i = 1:3
    ...
    fprintf("Patient hose transfer function for C_lung %f is: \n",C_lung(i))
    H_s(:,i)
    
    fprintf("Overall plant transfer function for C_lung %f is  is : \n",C_lung(i))
    B_s*H_s(:,i)
end
...
xlabel("Time[s]")
ylabel("Paw[mbar]")
title("Effect of Varying CLung on Paw")
for i = 1:length(C_lung)
